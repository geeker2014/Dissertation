...
#'            \code{lme}.
lmmSmooth2<- function(x, y, degree = 1, knots, num.knots, basis, random,
                     method = c("REML", "ML"), ...) {
  ...
  res <- list(x = x, fit = fit, y = y, knots = knots, degree = degree, 
              ...
              df.res = df.res)
  class(res) <- "lmmSmooth2"
  res
}


## Pedict method for objects of class "lmmSmooth2" ------------------------------
predict.lmmSmooth2 <- function(object, newdata,  
  ...
  if (interval != "none") {
    ...
    res <- list(fit = f.hat, 
                se = sqrt(object$var.components)[1] * sqrt(diag(S.mat %*% t(S.mat))))
  }
...
## Plot method for objects of class "lmmSmooth2" --------------------------------
plot.lmmSmooth2 <- function(object, ..., n = 500, lwd.smooth = 2, lty.smooth = 1, 
                           col.smooth = "black", knots = TRUE, 
                           col.knots = "grey", lwd.knots = 1, lty.knots = 3,
                           interval = c("none", "confidence", "prediction")) {
  
  ## Extract needed components from lmmSmooth2 object
  x <- object$x
  y <- object$y
  ...
  interval <- match.arg(interval)
  if (interval != "none") {
    conf <- predict.lmmSmooth2(object, newdata = newx, interval = interval)
    lines(newx, conf$lwr)
    lines(newx, conf$upr)
  }
}

## lmmSmooth2 method for invest function ----------------------------------------
invest.lmmSmooth2 <- function(object, y0, level = 0.95, mean.response = FALSE, 
  ...
  invEstFun <- function(x) {
    predict.lmmSmooth2(object, newdata = x) - y0
  }
  invLwrFun <- function(x) {
    predict.lmmSmooth2(object, newdata = x, interval = interval,
