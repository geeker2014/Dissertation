...
\usepackage{mathtools,graphicx,enumerate,float,booktabs,url,amsthm,framed,bm,amsfonts}
...
\graduation      {??}{March}{2013} % format is {DD}{Month}{YYYY} where
                                   % Month must be: March, June,
                                   % September, or December

\designator{AFIT/ENC/DS/14-01} % assigned by the graduate advisor in
...
\committee{ % Advisor must be listed first in the list of committee members
  {Christine Schubert Kabban, PhD (Chairman)},
...
\dean       {Adedeji Badiru} % only used for PhD dissertations
...
\abstract{Calibration is an important and controversial topic in statistics. In this dissertation, we consider both semiparametric calibration and the application of calibration to grouped data, both of which may be addressed through the use of the linear mixed-effects model. In Chapter~\ref{chap:nonparametric}, we propose a method for obtaining calibration intervals that have good coverage probability when the calibration curve has been estimated \textit{semiparametrically}. We also expand the traditional Bayesian approach to calibration by allowing for a semiparametric calibration curve. In Chapter~\ref{chp:cal-dependent}, we extend the usual methods for statistical calibration to the case of grouped data, that is, where observations can be categorized into a finite set of homogeneous clusters. An important characteristic of grouped data is that observations belonging to the same cluster are often similar, therefore, cannot be considered as independent; these data must be analysed in a way that accounts for this dependence (e.g., using mixed-effects models). We start by extending the classical Wald and inversion methods for obtaining calibration intervals, then we propose a new parametric bootstrap algorithm that, presumably, performs better for calibration in linear mixed-effects models by relaxing normality assumptions that may not be satisfied in practice. In Appendix~\ref{app:investr}, we describe the R package \texttt{investr} we have developed for analyzing calibration data.}
...
\DatesCovered{Oct 2010--Mar 2014} % First quarter of classes to Graduation
...
\ResponsiblePerson {Dr. Im N. Charge (ENG)}
\RPTelephone {(937) 255-3636 x0000 ImNCharge@afit.edu}  
...
Estimation of $\bm{\beta}$ in the linear model \eqref{eqn:linmod} can be carried out via least squares (LS). The ordinary LS estimator minimizes the sum of squares $S(\bm{\beta})$ defined by
\begin{equation}
\label{eqn:ss-linmod}
  S(\bm{\beta}) = \norm{\bc{Y} - \X\bm{\beta}}^2.
\end{equation}
The ordinary LS estimator of $\bm{\beta}$ is 
\begin{equation}
\label{eqn:beta-ols}
  \wh{\bm{\beta}} = (\X\trans\X)^{-1}\X\trans\bc{Y}
\end{equation}
If we make the additional assumption that the errors are normally distributed, then estimation can also be carried out by the method of maximum likelihood (ML). This has the benefit of simultaneously providing an estimator for both $\bm{\beta}$ and $\sigma_\epsilon^2$. To proceed, we need to maximize the likelihood
\begin{equation*}
...
\begin{equation*}
  \ell\left(\bm{\beta}, \sigma_\epsilon^2 | \bc{Y}\right) = -\frac{n}{2}\log\left(2\pi\right) - \frac{n}{2}\log\left(\sigma_\epsilon^2\right) - \frac{1}{2\sigma_\epsilon^2}\norm{\bc{Y} - \X\bm{\beta}}^2.
\end{equation*}
The derivatives of $\ell(\bm{\beta}, \sigma_\epsilon^2 | \bc{Y})$ with respect to the parameters $\left(\bm{\beta}, \sigma_\epsilon^2\right)$ are
\begin{align*}
  \frac{\partial\ell}{\partial\bm{\beta}} &= \frac{\X\trans(\bc{Y} - \X\bm{\beta})}{\sigma_\epsilon^2}\\
  \frac{\partial\ell}{\partial\sigma_\epsilon^2} &= \frac{\norm{\bc{Y} - \X\bm{\beta}}^2}{2\sigma_\epsilon^2} - \frac{N}{2\sigma_\epsilon^2},
\end{align*}
...
\end{align*}
Fortunately, for the linear model \eqref{eqn:linmod} with normal errors, the ML estimator of $\bm{\beta}$ is the same as the LS estimator. It is customary to adjust the ML estimator of $\sigma_\epsilon^2$ for bias by replacing it with $\wh{\sigma}_\epsilon^2 = \norm{\bc{Y} - \X\wh{\bm{\beta}}}^2/(n-p-1)$.
...
Let $\X_0$ be an arbitrary value of $\X$. Suppose we are interested in estimating the mean of $\mc{Y}$ given $\X_0$, $\E\left\{\mc{Y} | \X_0\right\} = \mu_0$. For the linear model \eqref{eqn:linmod}, the best linear unbiased estimator (BLUE) of $\mu_0$ is the fitted value $\wh{\mu}_0 = \X_0\trans\wh{\bm{\beta}}$. Furthermore, it is easy to show that
\begin{equation*}
...
\end{align}
where $\bc{Y}^{(0)} = \bc{Y} - \bm{\mu}(\bm{\beta}^{(0)})$, and $\X^{(0)} = \bm{D}(\bm{\beta}^{(0)})$. Equation~\eqref{eqn:ss-approximate} is of the same form as the sum of squares for the linear model \eqref{eqn:ss-linmod}. Based on this approximation, the LS estimate of $\bm{\beta}$ is then
\begin{equation*}
...
\end{equation*}
This process is iterated until a suitable convergence criterion is met. Just as for the linear model, if we assume the errors are normally distributed, then the ML estimate of  $\bm{\beta}$ is the same as the LS solution.
...
Let $\X = \left(x_1, \dotsc, x_n \right)\trans$ be a vector of predictor values and $\bc{Y} = \left( \mc{Y}_1, \dotsc, \mc{Y}_n \right)\trans$ be a vector of response variables. We assume that $\X$ and $\bc{Y}$ are related by the regression model
\begin{equation*}
  \bc{Y} = \mu(\X) + \bm{\epsilon}, \quad \bm{\epsilon} \sim \mc{N}(\bm{0}, \sigma_\epsilon^2\bm{I}),
\end{equation*}
where $\mu(\cdot)$ is an unknown smooth function. In this section, we discuss a technique for estimating $\mu(\cdot)$ nonparametrically, often referred to as \textit{scatterplot smoothing}, or just \textit{smoothing}. In particular, we will focus on an important class of smoothers called \textit{linear smoothers}. For linear smoothers, the prediction at any point $x$, is a linear combination of the response values: $\wh{\mu}(x) = \bm{\omega}\trans\bc{Y}$, where $\bm{\omega}$ is a constant that does not depend on the response vector $\bc{Y}$. The vector of fitted values $\wh{\bm{\mu}} = \left( \mu(x_1), \dotsc, \mu(x_n) \right)\trans$ can be written in matrix form as $\bm{S}\bc{Y}$, where $\bm{S} = \Big\lbrace_\text{row } \ell_{x_i}\trans \Big\rbrace_{i = 1}^n$ is an $n \times n$ \textit{smoother matrix}. Examples of linear smoothers include:
\begin{itemize}
...
\end{itemize}
The remainder of this section discusses spline smoothing, specifically, penalized regression splines (P-splines).  
...
\label{sec:lmms}
Mixed effects models (henceforth referred to as just mixed models) represent a large and growing area of statistics. In this section, we only summarize the key aspects of a special kind of mixed model called linear mixed model (LMM). The extension to nonlinear mixed effects models and generalized linear mixed effects models is discussed in  \citet{pinheiro_mixed_2009} and \citet{mcculloch_generalized_2008}, respectively. Mixed models are useful for describing \textit{grouped data} where observations belonging to the same group are correlated. One way of accounting for such correlation is by introducing random effects into the model which induces a particular correlation structure on the response vector. Different random effects structures induce different correlation structures on the response. 

The linear mixed model (LMM) extends the linear model \eqref{eqn:linmod-matrixform} to
\begin{equation}
...
\label{eqn:lmm-loglik-long}
  \ell\left(\bm{\beta}, \bm{\theta}\right) = -\frac{1}{2}\log\left|\bm{V}\right| - \frac{1}{2}\left(\bc{Y} - \X\bm{\beta}\right)\trans\bm{V}^{-1}\left(\bc{Y} - \X\bm{\beta}\right),
\end{equation}
where $\bm{\theta}$ is a vector containing the unique elements of $\bm{V}$. Equating the partial derivative of $\ell\left(\bm{\beta}, \bm{\theta}\right)$, with respect to the parameter $\bm{\beta}$, yields the so-called generalized least squares (GLS) estimator
\begin{equation}
\label{eqn:beta-gls}
  \wt{\bm{\beta}} = \left(\X\trans\bm{V}^{-1}\X\right)^{-1}\X\trans\bm{V}^{-1}\bc{Y}.
\end{equation}
Notice, however, that the GLS estimator---which happens to be the BLUE of $\bm{\beta}$---depends on the variance-covariance matrix $\bm{V}$. Since this is rarely available in practice, the usual procedure is to estimate $\bm{V}$ and then plug this into Equation~\eqref{eqn:beta-gls}. In other words, if $\widehat{\bm{V}}$ is an estimate of $\bm{V}$, then the estimated or empirical BLUE (EBLUE) of $\bm{\beta}$ is 
\begin{equation}
\label{eqn:beta-egls}
...
A technique known as \textit{best linear unbiased prediction} (BLUP) is commonly used to estimate the random effects $\bm{\alpha}$. It can be shown \citep{henderson_sire_1973} that the BLUE of $\bm{\beta}$ and the BLUP of $\bm{\alpha}$, denoted $\wt{\bm{\alpha}}$, simultaneously minimize
\begin{equation}
...
\begin{equation*}
  ...
  \end{bmatrix} = \bm{\Omega}(\bm{\Omega}\trans\bm{R}^{-1}\bm{\Omega} + \bm{D})\bm{\Omega}\trans\bm{R}^{-1}\bc{Y},
\end{equation*}
...
For the special case $\bm{R} = \sigma_\epsilon^2\bm{I}$ and $\bm{G} = \sigma_\alpha^2\bm{I}$, Equation~\eqref{eqn:henderson's-justification} reduces to the penalized sum of squares (PSS)
\begin{equation}
\label{eqn:lmm-pss}
  \frac{1}{\sigma_\epsilon^2}\norm{\bc{Y} - \X\bm{\beta} - \Z\bm{\alpha}}^2 +\frac{1}{\sigma_\alpha^2}\norm{\bm{\alpha}}^2.
\end{equation}
Similar to the EBLUE of $\bm{\beta}$, the estimated BLUP (EBLUP) of $\bm{\alpha}$ is just the BLUP $\wt{\bm{\alpha}}$ with $\bm{G}$ and $\bm{V}$ replaced with their respective estimates, $\widehat{\bm{G}}$ and $\widehat{\bm{V}}$:
\begin{equation}
\label{eqn:alpha-eblup}
  \wh{\bm{\alpha}} = \widehat{\bm{G}}\Z\trans\widehat{\bm{V}}^{-1}\left(\bc{Y} - \X\wh{\bm{\beta}}\right).
\end{equation} 
In a similar fashion, the BLUP and EBLUP of the mean response are given, respectively, by the equations
\begin{align}
  \widetilde{\bm{\mu}} &= \X\wt{\bm{\beta}} + \Z\wt{\bm{\alpha}}, \label{eqn:mu-blup} \\
  \wh{\bm{\mu}} &= \X\wh{\bm{\beta}} + \Z\wh{\bm{\alpha}}. \label{eqn:mu-eblup}
\end{align}
Note that the EBLUP $\wh{\bm{\mu}}$ is just the fitted values.

Obviously, estimating $\wt{\bm{\beta}}$ and $\wt{\bm{\alpha}}$, requires knowledge of the variance-covariance matrices $\bm{G}$ and $\bm{R}$, which are generally unknown. In practice, we often restrict these matrices to have a simple form, usually involving only a few unknown parameters, earlier denoted by $\bm{\theta}$. These parameters can be estimated via ML estimation. ML estimators of the variance components $\bm{\theta}$, however, tend to become badly biased as the number of fixed effects in the model increases. A more effective approach, known as restricted maximum likelihood (REML) estimation, is often used instead (see, for example, \citet[chap. 6]{mcculloch_generalized_2008}). 
...
\hlcom{## Set up Sigma from Equation (56)}
